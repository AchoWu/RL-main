#!/usr/bin/env bash
# Stop-Content Decoupled OPD：Skywork-OR1-Math-7B → DeepSeek-R1-Distill-Qwen-1.5B
# 严格对照 train_opd_skywork7b_to_r1qwen15b_reverse.sh；训练配置、数据划分、
# seed、学习率、步数和验证周期全部相同，仅将 KL 分成：
#   - content: teacher top-k 排除 EOS 后的 conditional reverse KL
#   - stop:    精确全词表 EOS 概率上的 forward Bernoulli KL
# 参考 train_opd.sh，公共配置：
...
#   - 不启用 EOS target-position mask，不启用 TVD gate。
set -euo pipefail
...
RUN_NAME="opd-skywork7b-to-r1qwen1.5b-stop-content-rkl-fkl-260817"

mkdir -p /group/40092/howu/RL-main/logs

# DeepSeek-R1-Distill-Qwen-1.5B 的 EOS token id = 151643。
# stop_content.stop_kl_type=forward 只改变停止决策的 KL 方向；内容仍为 reverse。
...
python examples/run_distillation_math.py \
    --config examples/configs/distillation_math_stop_content.yaml \
    ...
    loss_fn.stop_content.enabled=true \
    loss_fn.stop_content.eos_token_id=151643 \
    loss_fn.stop_content.stop_kl_type=forward \
    loss_fn.stop_content.stop_kl_weight=1.0 \
    loss_fn.stop_content.probability_eps=1.0e-7 \
